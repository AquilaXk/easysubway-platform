...
- A등급(운영 위험: deploy, rollback·standby promotion, backup·restore, observability(metric/log/alert/dashboard), auth·접근 제어(ingress·관측 스택 인증), secret·env allowlist, infra provisioning(Terraform), immutable Journey release tuple·release contract(`contracts/release/**`), CI/CD workflow 변경): .github/PULL_REQUEST_TEMPLATE/full.md 내용으로 교체합니다.
...
- [ ] 운영 위험 없음 (deploy/rollback/backup·restore/observability/secret·env allowlist/auth 아님)
