...
const backupRootId = "oci-data-volume-backup-control";
const backupRootPath = "infra/terraform/oci/data-volume-backup-control";
...
function recordApprovedReports(directory, findings = currentCheckovFindings) {
  ...
  recordCleanBackupReports(directory);
}
function recordCleanBackupReports(directory) {
  write(directory, "tflint-backup.sarif", tflintSarif());
  write(directory, "checkov-backup.sarif", checkovSarifRecords([]));
  write(directory, "checkov-backup.json", cleanCheckovSummary());
  recordScan({ reportDirectory: directory, scanner: "TFLINT", rootId: backupRootId, rootPath: backupRootPath, exit: 0, rawSarifPath: "tflint-backup.sarif", structuredJsonPath: null });
  recordScan({ reportDirectory: directory, scanner: "CHECKOV", rootId: backupRootId, rootPath: backupRootPath, exit: 0, rawSarifPath: "checkov-backup.sarif", structuredJsonPath: "checkov-backup.json" });
}
