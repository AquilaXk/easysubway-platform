...
const approvedSuppressionRows = [
  ...
  ["CKV_OCI_2", "storage.tf", "oci_core_volume.data[0]", "NOT_APPLICABLE_WITH_REASON", "pinned Checkov 3.3.9는 deprecated oci_core_volume.backup_policy_id만 검사하고 Oracle 권장 oci_core_volume_backup_policy_assignment graph를 인식하지 못함", "scanner 단독 결과는 exact assignment 존재를 분류하지 못하므로 deterministic one-volume/one-policy contract와 Terraform validate evidence가 필요함", "48", "[Resilience][Platform][P1] OCI data volume backup·restore contract로 CKV_OCI_2 해소", "Checkov가 oci_core_volume_backup_policy_assignment asset_id/policy_id graph를 인식하면 CKV_OCI_2 decision 삭제"],
