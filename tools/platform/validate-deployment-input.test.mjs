...
test("deployment runtime input inventory is closed and source-literal backed", () => {
  ...
  const entries = [
    ...
    ["source-free.k3s-runner.no-fallback-lifecycle", "tools/platform/run-k3s-journey-activation.mjs", "TARGET_JOURNEY_V3_REQUIRED", ["commitActiveServiceCas", "runPublicSmoke", "rollbackAttemptCount: 0", "fallbackZero: FALLBACK_ZERO", "K3S_PRECOMMIT_FAILED", "K3S_POSTSWITCH_FAILED"]],
  ...
  assert.equal(inventory.entries.length, 23);
  assert.equal(inventory.entries.filter(({ journeyV3Disposition }) => journeyV3Disposition === "TARGET_JOURNEY_V3_REQUIRED").length, 17);
  assert.equal(inventory.entries.filter(({ journeyV3Disposition }) => journeyV3Disposition === "LEGACY_NOT_JOURNEY_V3").length, 6);
  ...
  for (const marker of ["DEPLOY_REPO_URL", "SNAPSHOT_EVIDENCE_PATH", "EASYSUBWAY_ROUTE_V2_ORIGIN_SECRET"]) assert.equal(legacyTokens.some((token) => token.includes(marker)), true);
});
