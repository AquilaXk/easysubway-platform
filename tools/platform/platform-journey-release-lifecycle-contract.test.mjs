...
test("closed schema accepts only the exact lifecycle contract", () => {
  ...
  const driftCases = [
    ...
    (value) => { value.candidate.minimumInstanceCount = 1; },
    (value) => { value.candidate.maximumInstanceCount = 1; },
    (value) => { value.candidate.distinctFailureDomainsRequired = false; },
...
test("candidate state machine and active commit keep mixed traffic at zero", () => {
  ...
  assert.deepEqual(contract.candidate, {
    ...
    distinctFailureDomainsRequired: true,
    requiredSteps: [
